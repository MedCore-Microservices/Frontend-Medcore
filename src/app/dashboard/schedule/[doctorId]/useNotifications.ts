...
export function NotificationsProvider({ children }: { children: React.ReactNode }) {
  ...
  const push = useCallback((kind: NotificationKind, title: string, message: string, opts?: { ttlMs?: number }) => {
    ...
    // Auto-dismiss salvo que sea ERROR y ttl < 0
    if (ttl > 0) {
  ...
  const markAllRead = useCallback(() => {
    // Marcar leídas y remover para limpiar la bandeja visual
    setItems(prev => prev.filter(n => {
      const t = timers.current[n.id];
      if (t) {
        clearTimeout(t); delete timers.current[n.id];
      }
      return false; // eliminar todas
    }));
  }, []);

  const unreadCount = items.filter(i => !i.read).length; // siempre se eliminan al marcar, aquí es decorativo
