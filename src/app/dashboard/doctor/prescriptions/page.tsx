  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Hacer Prescripción</h1>
          <p className="text-gray-400">Selecciona un paciente para crear una nueva prescripción médica</p>
        </div>
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-gray-300">Edad Mínima</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-gray-300">Edad Máxima</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-gray-300">Tipo de Sangre</label>
            <select
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-xl text-gray-400">No se encontraron pacientes</p>
          ...
          <div className="grid grid-cols-1 gap-4">
            {patients.map((patient) => (
              <div
                key={patient.id}
                className="bg-gray-800 border border-gray-700 rounded-lg p-5 hover:border-blue-500 transition-colors"
              >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-2 text-sm">
                      <div>
                        ...
                        <span className="ml-2 text-white">{patient.identificationNumber}</span>
                      </div>
                      {patient.email && (
                        <div>
                          <span className="text-gray-400">Email:</span>
                          <span className="ml-2 text-white">{patient.email}</span>
                        </div>
                      )}
                      {patient.phone && (
                        <div>
                          <span className="text-gray-400">Teléfono:</span>
                          <span className="ml-2 text-white">{patient.phone}</span>
                        </div>
                      )}
                      {patient.age && (
                        <div>
                          <span className="text-gray-400">Edad:</span>
                          <span className="ml-2 text-white">{patient.age} años</span>
                        </div>
                      )}
                      {patient.gender && (
                        <div>
                          <span className="text-gray-400">Género:</span>
                          <span className="ml-2 text-white">{patient.gender}</span>
                        </div>
                      )}
                      {patient.bloodType && (
                        <div>
                          <span className="text-gray-400">Tipo de sangre:</span>
                          <span className="ml-2 text-white">{patient.bloodType}</span>
                        </div>
                      )}
                    </div>
                    {patient.allergies && (
                      <div className="mt-3 p-2 bg-red-900/20 border border-red-700 rounded">
                        <span className="text-red-400 font-medium">Alergias:</span>
                    ...
                    {patient.chronicDiseases && (
                      <div className="mt-2 p-2 bg-yellow-900/20 border border-yellow-700 rounded">
                        <span className="text-yellow-400 font-medium">Enfermedades crónicas:</span>
                        <span className="ml-2 text-yellow-300">{patient.chronicDiseases}</span>
                      </div>
                    )}
                  </div>
